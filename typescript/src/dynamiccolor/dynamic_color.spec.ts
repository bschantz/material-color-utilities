...
import {Hct} from '../hct';
import {SchemeContent} from '../scheme';
import {SchemeExpressive} from '../scheme';
import {SchemeFidelity} from '../scheme';
import {SchemeMonochrome} from '../scheme';
import {SchemeNeutral} from '../scheme';
import {SchemeTonalSpot} from '../scheme';
import {SchemeVibrant} from '../scheme';
...
describe('DynamicColor', () => {
  ...
  test('generates colors respecting contrast', () => {
    for (const scheme of schemes) {
      for (const pair of textSurfacePairs) {
        ...
        if (contrast < minimumRequirement) {
          test.fails(`${fgName} on ${bgName} is ${contrast}, needed ${
              minimumRequirement}`);
        }
      }
    }
  ...
  test('constraint conformance test', () => {
    /*
      Tone delta pair:
        ...
        if (cstr.kind === 'Contrast') {
          ...
          for (const pair of pairs) {
            ...
            if (contrast < minRequirement - contrastTolerance &&
                minRequirement <= 4.5) {
              // Real fail.
              assert.fail(`Contrast ${fore} ${ftone.toFixed(prec)} ${back} ${
                  btone.toFixed(
                      prec)} ${contrast.toFixed(prec)} ${minRequirement} `);
            }
            if (failing && minRequirement > 4.5) {
              assert.fail(`Contrast(stretch-goal) ${fore} ${ftone.toFixed(prec)} ${
          ...
          for (const pair of pairs) {
            ...
            if (observedDelta < cstr.delta! - 0.5 /* lenient */) {
              // Failing
              assert.fail(`Delta ${fore} ${ftone.toFixed(prec)} ${back} ${
                  btone.toFixed(
