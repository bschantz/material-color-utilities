...
describe('dislike analyzer', () => {
  test('likes Monk Skin Tone Scale colors', () => {
    ...
    for (const color of monkSkinToneScaleColors) {
      expect(DislikeAnalyzer.isDisliked(Hct.fromInt(color))).equals(false);
    }
  });
  ...
  test('dislikes bile colors', () => {
    ...
    for (const color of unlikable) {
      expect(DislikeAnalyzer.isDisliked(Hct.fromInt(color))).equals(true);
    }
  });
  ...
  test('makes bile colors likable', () => {
    ...
    for (const color of unlikable) {
      ...
      expect(DislikeAnalyzer.isDisliked(hct)).equals(true);
      const likable = DislikeAnalyzer.fixIfDisliked(hct);
      expect(DislikeAnalyzer.isDisliked(likable)).equals(false);
    }
  });

  test('likes tone 67 colors', () => {
    const color = Hct.from(100.0, 50.0, 67.0);
    expect(DislikeAnalyzer.isDisliked(color)).equals(false);
    expect(DislikeAnalyzer.fixIfDisliked(color).toInt()).equals(color.toInt());
  });
});
